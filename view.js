...
;(function(window, document) {
	...
	class View extends EventEmmitter {
			...
			btns.forEach(function(btn) {
				btn.addEventListener('click', function() {
					...
					if (action === 'delete') {
						...
						self.emit('complete', {elem: item, id, completed: item.classList.contains('completed')});
					} else if (action === 'edit') {
		...
		addTask (taskInfo, sortType) {	// Доработать функцию createElement
			...
			const taskEl = createElement('div', ['todos-item', taskInfo.completed ? 'completed' : ''], {'data-id': taskInfo.id}, createElement('div', ['todos-item__title'], {}, taskInfo.title), taskEditInput, createElement('div', ['todos-item__actions'], {}, taskEditBtn, taskCompleteBtn, taskDeleteBtn));
		...
		completeTask (item, completed) {
			(!completed) ? item.classList.add('completed') : item.classList.remove('completed');
		}
		...
		filterTasks (type) {
			...
			if (type === 'active') {
				...
				for (let i = 0; i < all.length; i++) {
					...
					numberOfFilteredTasks = all.length;
				}
			}

			this.renderTotalTasksNumber(numberOfFilteredTasks);
		}
		...
		sortTasks (type) {
			this.setActiveOption(this.sortSelect.options, type, 'sort');
		}
		...
		showAlert (props) {	// использовать в контроллере
			const alert = document.querySelector('.alert');

			alert.classList.add(`alert-${props.type}`);	// error, warning, success, info
			alert.textContent = props.message;
			alert.style.display = 'block';

			setTimeout(() => {
				alert.style.display = 'none';
				alert.classList.remove(`alert-${props.type}`);
				alert.textContent = '';
			}, props.time);
		}
	}
...
// 		// --- Основные операции с задачей ---
// 		addTask (taskInfo, sortType = 'new') {	// Доработать функцию createElement
// 			const taskEditBtn = createElement('span', ['editBtn', 'task-btn'], {'data-action': 'edit'}, createElement('i', ['fa', 'fa-pencil']));
// 			const taskCompleteBtn = createElement('span', ['completeBtn', 'task-btn'], {'data-action': 'complete'}, createElement('i', ['fa', 'fa-check']));
// 			const taskDeleteBtn = createElement('span', ['deleteBtn', 'task-btn'], {'data-action': 'delete'}, createElement('i', ['fa', 'fa-close']));
// 			const taskEditInput = createElement('input', ['todos-item__input', 'hide'], {value: taskInfo.title});
